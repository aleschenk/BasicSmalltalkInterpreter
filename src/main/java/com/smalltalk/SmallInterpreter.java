...
import com.smalltalk.image.FileImageXml;
import com.smalltalk.objects.Sema;
import com.smalltalk.objects.SmallByteArray;
import com.smalltalk.objects.SmallException;
import com.smalltalk.objects.SmallInt;
import com.smalltalk.objects.SmallJavaObject;
...
public class SmallInterpreter implements Serializable {
	...
	public SmallInt newInteger(int val) {
		if ((val >= 0) && (val < 10))
	...
	private SmallObject methodLookup(SmallObject receiver, SmallByteArray messageSelector, SmallObject context,
		...
		return methodLookup(receiver, new SmallByteArray(messageSelector.objClass, "error:"), context, arguments);
	}
	...
	// execution method
	SmallObject execute(SmallObject context, final Thread myThread, final Thread parentThread) throws SmallException {
		...
		outerLoop: while (true) {
			...
			innerLoop: while (true) {
				...
				case 9: // SendMessage
					...
					contextData[4] = (bytePointer < 10) ? smallInts[bytePointer] : new SmallInt(IntegerClass,
							bytePointer);
					...
					if ((selectorCache[high] != null) && (selectorCache[high] == returnedValue)
						...
						method = methodLookup(arguments.data[0].objClass, (SmallByteArray) literals[low], context,
								arguments);
						lookup++;
				...
				case 11: {// SendBinary
					if ((stack[stackTop - 1] instanceof SmallInt) && (stack[stackTop - 2] instanceof SmallInt)) {
				...
				case 13: // Do Primitive, low is arg count, next byte is number
					...
					case 29: { // image save
						...
						try {
							...
							// // oos.writeObject(this); write one by one to avoid serialization
...
//							FileImageXml.write(name, this);
							FileImageBin.write(name, this);
						} catch (Exception e) {
					...
					case 85: { // new slider
						...
						final JScrollBar bar = new JScrollBar(((orient == trueObject) ? JScrollBar.VERTICAL
								: JScrollBar.HORIZONTAL), min, 10, min, max);
